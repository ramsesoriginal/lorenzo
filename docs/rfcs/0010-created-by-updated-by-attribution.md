...
- **`entity_prototype`/`character_player`** — **not** "no write path," despite an earlier draft of this RFC claiming exactly that: [RFC 0005](0005-item-and-item-instance-crud-api.md)'s `POST /items`/`POST /item-instances` create `EntityPrototype` rows, and [RFC 0007](0007-user-player-character-crud-api.md)'s `POST /characters` and its player-roster sub-resource create `CharacterPlayer` rows — the same category of mistake this RFC already caught itself making once, above, for `campaign_gm`/`tenant_admin_campaign_opt_out`. Still excluded here, but for the same reason as `containment`/`ownership` above, not for lacking a write path: both are pure composite-PK join tables with no data beyond the relationship itself ([ADR 0015](../adr/0015-entity-prototype.md)'s own words for `entity_prototype`), neither has `created_at`/`updated_at` today either, and bolting on half an attribution story ahead of a fuller "who changed this relationship, and when" mechanism would pre-empt that rather than build toward it.
