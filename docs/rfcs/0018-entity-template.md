...
**Every information slot's content is filtered through the same `information_visibility.py` check every other information read path already uses** ([ADR 0028](../adr/0028-knowledge-and-group-membership.md)'s addendum) — a GM-only note bundled into a template slot is exactly as gated as it would be reading `Information` directly; a caller who can't see it gets that slot omitted or empty in the assembled response, never silently included. This is a hard requirement of this RFC, not an implementation detail left for the ADR to discover: every prior read path that skipped this check (`GET /entities/{id}`, item descriptions, payload serving) turned out to be a real, live information-disclosure bug once checked, and a template is structurally a fourth such path — it doesn't get to be the one exception.
...
**Authoring a template is tenant-admin tier** — the same bar [RFC 0016](0016-stats-computed-values-and-crud-api.md)'s `stat_group`/`stat_definition` already use ([ADR 0037](../adr/0037-effective-stat-resolution.md)), not self-or-managed: a template is exactly that kind of shared, defined-once, built-on-top-of-by-everything-else concern, not something scoped to one entity's own owner. This is deliberately a placeholder, not a final answer — a "repository author" role, tied to the still-open "repository" question below, is expected to eventually own template (and computed-stat) authoring, with a tenant admin *subscribing to* (copying from) a repository rather than authoring from scratch. That role doesn't exist yet; tenant-admin is what this RFC actually builds, named here so it isn't mistaken for the long-term design.
...
- Template assembly is a third layer of live, uncached computation stacked on top of prototype resolution ([ADR 0037](../adr/0037-effective-stat-resolution.md)/[0039](../adr/0039-generic-effective-stat-view.md)) and computed-stat evaluation ([RFC 0016](0016-stats-computed-values-and-crud-api.md)) — real materialization/caching is a genuine, known future need across all three layers, not solved by this RFC. Named explicitly so it isn't mistaken for a closed question.
