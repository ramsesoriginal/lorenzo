...
### 4. Campaign-relevance of world content ("facts")

Not from this RFC's original scope — raised while designing campaign-scoped GM visibility ([RFC 0009](0009-campaign-scoped-gm-visibility.md)). That RFC's reachability walk (a GM sees whatever's owned/contained by their own campaign's characters) is sufficient for inventory specifically, because an item's presence in a campaign is always mediated through a character by construction. It doesn't generalize to world content nothing has touched yet: an NPC no one's met, a location no one's visited, an item sitting in an unvisited dungeon's chest, a rumor circulating with no fixed origin, the description of a specific tree somewhere in the world. None of these are reachable from any character, yet they're clearly "part of" some campaign's unfolding story (or not yet part of any) in a way this RFC's `entity`/`information` model has no way to express — there's no `entity_id`-to-`campaign_id` linkage anywhere, deliberately, since `entity` is tenant-scoped, not campaign-scoped ([RFC 0002](0002-campaign-player-character-model.md)).

One direction, not designed: a "facts" mechanism — an explicit, campaign-scoped (or knowledge-scoped) linkage recording that a given entity or piece of information is live/relevant/discoverable within a specific campaign's story, independent of any character having encountered it yet, with a GM presumably "revealing" facts into actual `knowledge` as the story progresses. Related to, but distinct from, open question #1 above: that question is about how multiple knowers relate to the same truth; this one is about which campaign(s) a truth is even in scope for in the first place. Out of scope for the items vertical slice (inventory is always character-attached, so the simpler reachability walk in RFC 0009 covers it) and for anything currently planned — recorded here so it isn't lost, not as a commitment to build it.
