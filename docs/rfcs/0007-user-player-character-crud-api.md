...
Status: proposed — builds on [RFC 0004](0004-user-membership-player-character-gm-read-api.md)'s read schemas and character table, and [RFC 0005](0005-item-and-item-instance-crud-api.md)'s write-API conventions; no schema changes of this RFC's own, but depends on RFC 0004's migration (the `character` table, `owner_player_id`'s move, `character_player`/`group_member`'s retargeted FKs) having landed first
...
- No migration of this RFC's own — but it now depends on [RFC 0004](0004-user-membership-player-character-gm-read-api.md)'s migration having landed first (`character`, `owner_player_id`'s move off `being`, `character_player`/`group_member`'s retargeted FKs), unlike this RFC's first draft, which depended on `app_user`/`membership`/`player`/`being`/`character_player` needing nothing further.
