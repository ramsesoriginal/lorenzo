...
**Fully destroying a `being`/`entity`, and creating a bare `being` with no `character` row.** Neither is exposed anywhere in this RFC — both are generic being/entity-lifecycle concerns, not character-specific ones, and belong to whatever future RFC covers generic entity CRUD (the same boundary [RFC 0005](0005-item-and-item-instance-crud-api.md) already draws for items). A GM who wants a truly disposable, never-to-be-a-character NPC has no API path to create just that today, same as before this revision — not a regression, just an unaddressed gap this RFC doesn't newly create.
...
**Promoting an existing `being` into a `character`, rather than only ever creating both fresh together.** A plausible, likely-wanted real scenario: an unnamed background NPC turns out to matter and a GM wants to give it the full character treatment retroactively. `POST /characters` as designed above can't do this — it always creates a brand-new `Entity`+`Being`+`Character` together, never attaches a `Character` to a `being` that already exists. A separate `PUT /tenants/{tenant_id}/beings/{id}/character` (or similar) could add this later without redesigning anything here; not built now since it wasn't asked for and `being`-level addressing isn't otherwise exposed by this RFC at all.
