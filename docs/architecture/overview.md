...
- **Multi-tenancy**: [ADR 0002](../adr/0002-multi-tenancy-shared-schema-rls.md) — shared schema + PostgreSQL row-level security. The app's own connection is now a restricted, non-superuser role RLS actually applies to ([ADR 0021](../adr/0021-restricted-app-role-for-rls-enforcement.md)) locally and in CI; production still needs its live Neon role/secret rotated to match (tracked as a deliberate, human-confirmed follow-up — see [docs/operations/deployment-setup.md](../operations/deployment-setup.md)'s rotation section).
...
- **Auth**: [ADR 0009](../adr/0009-identity-provider-authgear.md)/[ADR 0023](../adr/0023-authgear-token-verification.md) — Authgear as the identity provider, with real OIDC bearer-token verification built and required on every tenant-scoped route.
- **Deployment**: [ADR 0011](../adr/0011-deploy-target-cloud-run-neon.md) — Google Cloud Run + Neon, continuously deployed on every push to `main` that touches `apps/api/` (`.github/workflows/deploy-api.yml`). Last confirmed working end to end for the pre-domain-model, pre-auth baseline; the domain model, RLS restricted role, and Authgear integration built since then haven't been re-verified against a live deploy, and production Authgear env vars aren't wired into the deploy workflow yet either. See [docs/architecture/deployment.md](deployment.md) for the components and how they connect, and [docs/operations/deployment-setup.md](../operations/deployment-setup.md) for the one-time account setup.

## Roadmap

Tracked as [GitHub issues/milestones](https://github.com/ramsesoriginal/lorenzo/issues) once there's something to track. Both vertical slices that were in progress are now built: simple inventory management (entity/stats/information/payloads/prototypes/containment/item/item_instance, ADRs 0012-0019, plus a read-only REST API over all of it, [ADR 0020](../adr/0020-rest-api-tenant-scoping-and-schemas.md)) and auth/users/campaigns/GM (Authgear, User/Tenant/Membership, Campaign/Player, Character/ownership, Campaign GM/orga — ADRs 0009/0021-0026, RFC 0002 accepted). Both currently live on feature branches (`feat/inventory-management`, `feat/auth-users`), not yet merged to `main`. Remaining, not yet built: RFC 0001's `knowledge`/redaction system, a fuller CRUD REST surface beyond read-only, campaign-scoped REST routes (the access rule itself is built - `campaign_access.can_access_campaign()` - just not wired to a route), and cross-tenant repositories ([docs/domain/repositories.md](../domain/repositories.md)).
