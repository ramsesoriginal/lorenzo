...
The first client app on top of all this, [`apps/loot-bot`](../../apps/loot-bot) ([ADR 0042](../adr/0042-loot-bot-stack-linking-and-isolation.md)), is under construction in parallel with the CRUD API work above — it depends on a small `apps/api` addition (resolving "which characters does this user control," and letting a player read their own inventory without a tenant-wide `Membership` row) that isn't built yet either.
