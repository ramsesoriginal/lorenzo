...
Three independent triggers can fire off the same push to `main`: `release.yml` (always, proposing/updating a version bump), `deploy-api.yml` (only if the push touches `apps/api/**`), and `deploy-loot-bot.yml` (only if it touches `apps/loot-bot/**`, [ADR 0045](../../adr/0045-loot-bot-http-interactions-and-cloud-run-deploy.md)). None depend on each other — a release PR being merged doesn't itself deploy anything, and a deploy doesn't wait for a release to be tagged. See [docs/operations/releasing.md](../../operations/releasing.md) and [docs/architecture/deployment.md](../deployment.md).
