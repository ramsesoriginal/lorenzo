...
- **Database**: Postgres via `infra/docker-compose.yml`, on port `55432` (not `5432`, deliberately clear of any other local Postgres). `.env` (copied from `.env.example`) points `DATABASE_URL` at it.
...
| Migrations | Nothing to run yet — no domain tables exist | Applied automatically, pre-deploy, every push |
| Access | Whatever's on your machine | Public, unauthenticated by design (`--no-invoker-iam-check`) — there's no app-level auth yet either way; see [ADR 0009](../adr/0009-identity-provider-authgear.md) |
