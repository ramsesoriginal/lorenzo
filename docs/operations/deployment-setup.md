...
See [ADR 0011](../adr/0011-deploy-target-cloud-run-neon.md) for why and [docs/architecture/deployment.md](../architecture/deployment.md) for how the pieces connect once this is done. **Confirmed working end to end** (2026-09-09): the full domain model, the restricted `lorenzo_app` role (RLS genuinely enforced - checked directly, not assumed), and production Authgear Cloud have all been verified together against the live deployed Cloud Run service, via a real login, a real token, and a real `GET /me` round-trip. `PROJECT_ID` must be globally unique across *all* of GCP, not just this repo — `lorenzo-api` was taken, `lorenzo-medici-api` wasn't; pick your own. Getting here took a few real fixes, each only surfaced by an actual run (a missing `README.md` in the Docker build context, asyncpg rejecting two of Neon's default libpq-only query params, the container ignoring Cloud Run's `$PORT`, new Cloud Run services being private by default, Neon's control-plane password-strength check rejecting a role's password when set via dynamic SQL) - see `deploy-api.yml`'s history for specifics.
...
`Settings` needs `authgear_issuer`/`authgear_jwks_url`/`authgear_audience` ([ADR 0023](../adr/0023-authgear-token-verification.md)) plus `authgear_userinfo_url` ([ADR 0075](../adr/0075-sync-email-via-userinfo-not-access-token.md)) pointed at a real project, not the `http://localhost:4000` placeholders. Authgear Cloud, not self-hosted (ADR 0027 — self-hosting would need its own Postgres + Redis + more, no free tier fits that the way Cloud Run/Neon do):
...
3. From that application's **Endpoints** section, copy the issuer URL. Fetch `<issuer>/.well-known/openid-configuration` to find `jwks_uri` and `userinfo_endpoint`.
4. This becomes `AUTHGEAR_ISSUER` and `AUTHGEAR_JWKS_URL` below, and `userinfo_endpoint` becomes `AUTHGEAR_USERINFO_URL` (ADR 0075 — this is what actually populates `User.email`, since the access token itself never carries an `email` claim). `AUTHGEAR_AUDIENCE` is the *same* issuer URL, not the client id — access tokens carry the project endpoint as `aud`, not an OIDC client id; that distinction only applies to ID tokens (see ADR 0023).
...
## Cloudflare Pages (apps/account-hub)

`apps/account-hub` deploys as a plain static build (`astro build` → `dist/`) — no container, no database, per [ADR 0004](../adr/0004-static-astro-frontend.md). **Via Cloudflare's own Git integration, not a GitHub Actions deploy step** — see [ADR 0071's addendum](../adr/0071-account-hub-stack-auth-deploy.md#addendum-2026-09-19-cloudflare-pages-git-integration-not-wrangler-action-token-upload): Cloudflare's current Pages onboarding leads with Connect to Git rather than surfacing an account ID for a token-based upload flow, so this app has no `deploy-account-hub.yml` — Cloudflare builds and deploys directly from pushes to `main`, and `ci.yml`'s own auto-discovered matrix already covers `mise run lint`/`test` for it on every PR.

1. Sign up at [cloudflare.com](https://cloudflare.com) (free tier) if you haven't already.
2. **Workers & Pages → Create → Pages → Connect to Git**, authorize Cloudflare's GitHub App for the `ramsesoriginal/lorenzo` repo (can scope the App to just this one repo), then select it.
...
`apps/account-hub` needs its **own** Authgear "Single Page Application" (public/PKCE) client — distinct from `apps/api`'s dev-token-script client and from `apps/loot-bot`'s own confidential client, exact-match redirect URIs per client:

1. **Authgear Portal → your production project → Applications → New Application → Single Page Application.**
2. **Authorized Redirect URIs**: add `https://lorenzo-account-hub.pages.dev/auth/redirect/` (the production Pages URL once step 3 above exists) and, for local dev, `http://127.0.0.1:4322/auth/redirect/`. **The trailing slash is required, not optional** — Cloudflare Pages 308-redirects a bare `/auth/redirect` request to add one before this app's own JS runs, so `auth.ts` requests the slash form itself; a no-slash registered URI causes the token exchange to fail with `invalid redirect URI`.
3. **Authorized Post-Logout Redirect URIs**: same origins, no path (`https://lorenzo-account-hub.pages.dev`, `http://127.0.0.1:4322`) — `auth.ts` calls `logout({ redirectURI: window.location.origin })`.
...
1. **Authgear Portal → your production project → Applications → New Application → Single Page Application.**
2. **Authorized Redirect URIs**: add `https://lorenzo-inventory-web.pages.dev/auth/redirect/` (the production Pages URL once step 3 above exists) and, for local dev, `http://127.0.0.1:4321/auth/redirect/`. **The trailing slash is required, not optional** — see `apps/account-hub`'s own note above; the same Cloudflare Pages behavior and the same `auth.ts` fix apply here.
...
2. **Pick credentials for the new restricted role** - a username (e.g. `lorenzo_app`) and a freshly generated password. In principle you don't need to create this role in Neon's console yourself: the migration in [`8aced4b80842_create_restricted_lorenzo_app_role.py`](../../apps/api/migrations/versions/8aced4b80842_create_restricted_lorenzo_app_role.py) reads whatever username/password `database_url` carries and creates exactly that role, idempotently, the next time migrations run. **In practice, against real Neon, this rejected every password tried** with a control-plane `"insecure password"` error regardless of actual strength - the working theory is that Neon validates passwords by inspecting literal SQL text, and this migration's `CREATE ROLE` is built dynamically inside a `DO`/`EXECUTE format(...)` block specifically to get `IF NOT EXISTS` semantics, which hides the real password from that check. The fix that actually worked: create the role directly via a plain, literal `CREATE ROLE ... LOGIN PASSWORD '...' NOSUPERUSER NOBYPASSRLS NOCREATEDB NOCREATEROLE NOREPLICATION` statement in Neon's own SQL editor first (with the same username/password you're about to put in `DATABASE_URL`) - once the role already exists, the migration's own `IF NOT EXISTS` check skips straight to the `GRANT` statements, which don't touch passwords at all. Rewriting the migration itself to avoid this is tracked as a follow-up, not yet done.
