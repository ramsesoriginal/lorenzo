...
`apps/loot-bot`, `apps/inventory-web`, and `apps/account-hub` are also built on top of `apps/api` — see each app's own README for how to run it locally. Everything beyond these four is still unbuilt — see [docs/guides/adding-an-app.md](adding-an-app.md) for what that takes, and [docs/architecture/overview.md](../architecture/overview.md) for the intended shape of the whole system.
