...
- [0091 - Tenant OWNER joins ORGA in the information-visibility bypass](0091-owner-joins-orga-in-the-information-visibility-bypass.md) — amends ADR 0035/0028: an OWNER reads GM-only information like an ORGA (per-campaign opt-out unchanged), found by ADR 0085's export audit; numbered past 0090, claimed on other branches (loot-bot, account-hub) at the time this was written
- [0092 - Shareable, expiring campaign invite links](0092-campaign-invite-links.md) — accepts RFC 0019: hashed 256-bit tokens, required expiry, optional use cap, player-only, login to redeem, edge rate rule plus in-process backstop
