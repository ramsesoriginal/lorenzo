...
- [0085 - account-hub: tenant/campaign pictures, an unread-notification badge, and a cross-tenant access overview](0085-account-hub-pictures-unread-badge-and-access-overview.md) — numbered past 0084, already claimed by the in-flight `feat/api-tenant-accountability-docs` work at the time this was written; [RFC 0019](../rfcs/0019-account-hub-pictures-overview-and-membership-safety.md) sub-slices 1-3, correcting two of that RFC's assumptions once checked against this app's real source (no shared nav exists - `index.astro` is the hub; `GET /me` alone can't name tenants/campaigns - `/tenants`' own `listMyTenants`/`listTenantCampaigns` aggregation is reused instead)
