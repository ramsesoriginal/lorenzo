...
- [0084 - Activity log coverage policy, OWNER/ORGA-only reads, and a member-removal notice](0084-activity-log-coverage-and-member-removal-notice.md) — states what ADR 0063's log is for and the rule for what it records (any actor, GMs included); narrows reads to tenant OWNER/ORGA (breaking for ADR 0083's account-hub view); adds `detail` to `membership.deleted` plus a removal notification
- [0085 - Tenant data export: audit of existing reads, three small missing read endpoints, and a runbook](0085-tenant-data-export-audit-and-runbook.md) — proposed; finds stat group/definition listing and knowledge grants unreachable, flags a possible OWNER-vs-ORGA visibility hole, defers the runbook to the implementation slice
- [0086 - `GET /me/managed` and a `since` filter on `GET /me/notifications`](0086-managed-scope-aggregate-and-notifications-since-filter.md) — proposed; cross-tenant "what do I run" index and an inclusive `since` timestamp filter
