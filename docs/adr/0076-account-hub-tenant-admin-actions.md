...
## Addendum: editing an existing campaign

Manual testing immediately surfaced the gap [RFC 0014](../rfcs/0014-account-hub-tenant-admin-and-roster-management.md) itself named ("editing a campaign's own fields after creation... genuinely out of scope for this RFC, not designed here, until asked for") - now asked for. Same actors, same page, same gate as creation (`owner`/`orga`), so this is an addendum here rather than a new RFC/ADR: an "Edit" toggle per campaign row reveals the same field set as the create form, pre-filled from `GET /tenants/{id}/campaigns/{id}` (`CampaignOut` - `CampaignSummaryOut` itself lacks `description`, so this needs the one extra fetch, only on toggling edit open, not on every page load), submitting only the changed fields via `PATCH /tenants/{id}/campaigns/{id}` (`CampaignUpdate`, `exclude_unset` semantics, same pattern `ProfileUpdate`/`CharacterUpdate` already established). `CampaignUpdate.if-match` (optional, ETag-based concurrency) is skipped for this slice, same simplicity call every other write action in this app has already made.
