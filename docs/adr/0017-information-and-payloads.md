...
- `payload_document(payload_id, tenant_id, data, filename)` - same `BYTEA` reasoning as `payload_picture`. Filename rather than a MIME type, per what was actually asked for; a content-type column is a natural addition when a serving endpoint actually needs one, not before.
