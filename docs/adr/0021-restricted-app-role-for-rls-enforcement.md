...
- **Done**: the production secret rotation was completed and confirmed directly against the live Neon role - `rolsuper=f`, `rolbypassrls=f`, `rolcreatedb=f`, `rolcreaterole=f`, `rolreplication=f` - not just deployed. See [docs/operations/deployment-setup.md](../operations/deployment-setup.md) for the rotation runbook and the real "insecure password" control-plane issue hit doing it. RLS now genuinely enforces tenant isolation in production, not only in CI/local dev.
