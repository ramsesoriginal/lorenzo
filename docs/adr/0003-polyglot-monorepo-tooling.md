...
- **release-please** (manifest mode) for independent per-package SemVer, Keep a Changelog-style changelogs, and GitHub Releases — `apps/api` is registered (`release-type: python`, starting `0.1.0`); a package entry gets added as each further app/package is registered. The automation itself hasn't run yet — it only triggers on pushes to `main`, and this work is still on a feature branch.
