# 0063 - Tenant activity log (first slice)
...
New `audit_log(id, tenant_id, actor_id, action, target_type, target_id, detail, created_at)`. `tenant_id` is **not nullable** for this slice - platform-scope events (e.g. account suspension, ADR 0057) are explicitly out of scope, not silently dropped (see below), so there's no NULL case to design around yet. `action`/`target_type` are free text (`"membership.created"`, `"campaign.deleted"`, ...) - the same `Information.type`/`Notification.type` precedent ADR 0017/0058 already established: caller-chosen categorization, not a fixed schema-level discriminant. `actor_id` is `ON DELETE SET NULL`, matching `created_by` everywhere else. RLS is the ordinary `tenant_id = current_setting('app.tenant_id')` shape (no self-access clause needed, unlike `notification` - this is inherently a per-tenant admin view, not a cross-tenant personal inbox).
...
- `create_membership`/bulk create (ADR 0062), `update_membership`, `delete_membership`
- `create_campaign`, `delete_campaign`
- `grant_campaign_gm`, `revoke_campaign_gm`

## Not in scope

Named explicitly, not hidden: platform-level events (suspend/unsuspend - no `tenant_id` to attach them to under this design); item/character/information mutations; picture uploads; notification sends. If platform-wide audit is wanted later, it needs a nullable `tenant_id` and a dedicated admin read-path (the same RLS-visibility problem ADR 0058 solved for `notification` with its self-access clause - this table doesn't need that complexity yet, since nothing in it is meant to be read cross-tenant).
