...
## Addendum: migrated to the API feed

RFC 0022 landed as [ADR 0099](0099-player-facing-change-feed.md) (`GET /me/changes`, across every client, every source), exactly the trigger this ADR's own Consequences section named. `/changes` now reads that instead of a bot-recorded log, as planned.

- **`character-events.ts`, every `recordCharacterEvents` call site (`/give`, `/give-bulk`, `/reassign`, `/award`, `/confiscate`, `/drop` take and apply-claims), and the `character_event` table are gone.** Migration `0007` drops the table; `0006`'s own `CREATE TABLE` is left untouched in history, per this repo's forward-only migration convention. `changes_seen` (the "last looked" marker) is kept unchanged — the API has no stored read-position of its own, only a `since` filter, so this bot still needs somewhere to remember it.
- **The coverage note is gone.** `/changes` now sees everything: `apps/inventory-web`, `apps/account-hub`, the API directly, every source ADR 0099 covers.
- **Retention moved server-side.** `GET /me/changes` prunes the caller's own rows past 90 days on read (ADR 0099); this bot no longer prunes anything itself.
- **The kind vocabulary changed.** The old table above was per-*command* (`gave`, `given-in-bulk`, `reassigned`, `awarded`, `confiscated`, `took`, `claim-honored`); the API's is per-*holder-set-diff*, the same seven kinds regardless of which command caused them (`received`, `given_away`, `moved`, `split`, `merged`, `renamed`, `deleted` — see ADR 0099's own "Who gets a row" section). `format-changes.ts`'s `describeChange` renders these; the by-command table above no longer describes what ships.
- **The actor can no longer be named, even when ADR 0099 marks it visible.** The old bot-recorded wording named the *character* on the other end of a transfer ("Frodo gave Torch to Sam") — always available, since every character's name is already one `getCharacterName` call away. `EntityChangeOut.actor_user_id` is a different thing: the Authgear user id of whoever made the change, and this bot has no endpoint that turns an arbitrary user id into a display name (unlike a character id — there is no `GET /users/{user_id}` an ordinary player can call, per ADR 0023's non-enumerable convention). `describeChange` can only fall back to "another player" when visible and silence when not, which is still meaningfully different (a peer's action vs. a GM's or administrator's) but is a real loss of specificity from before. Naming the actor would need its own API surface and isn't built here.
- **Tenant-scoped client-side, like `listMyUnreadNotifications`.** `GET /me/changes` spans every tenant the caller's account has rows in (ADR 0099); `lorenzo-client.ts`'s `listMyChanges` filters to this bot's own tenant before returning, the same shape the notification-DM bridge (ADR 0095) already uses.
- **Not built, still:** per-character filtering, and naming the actor (now blocked on a name-resolution endpoint this bot doesn't have, not just on a product decision).
