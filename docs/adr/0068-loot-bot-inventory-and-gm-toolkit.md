...
25 new-or-changed commands across this ADR is enough that "what can this bot even do" stopped being answerable by memory. `/help` lists every command grouped into a small, hand-maintained set of categories (`commands/help.ts`'s own `CATEGORIES`, by command name - nothing about a `Command` object says what it's "for," so this one piece of metadata has no other home), or, given an optional `command` option, shows one command's full option list.

Deliberately reads the *real* command list rather than a second, hand-written name/description table that could quietly drift from what's actually registered: `commands/index.ts`'s `dispatchInteraction` now injects the full `commands` array into `ctx.commands` before running any command (`CommandContext` gains this field, optional - the ctx built in `interactions-route.ts`, and every existing test's own fixture ctx, doesn't know the list, only `dispatchInteraction` does, right where it already has it in scope). `/help`'s own detail view reads a command's `SlashCommandBuilder.toJSON()` directly - the exact same object `scripts/register-commands.ts` sends to Discord - so the options shown can never disagree with what a player actually sees when typing the command. A command missing from `CATEGORIES` still shows up, under "Other," rather than silently vanishing - a missed edit is a cosmetic gap, not a silent omission. No `/link` gate, no API call - same "works for anyone, no dependency" precedent `/ping` already set.
