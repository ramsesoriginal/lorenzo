...
- Same RLS caveat every table up to this point carried, now resolved: `security_invoker = true` was real but unenforced groundwork until the app's DB role stopped being a superuser ([ADR 0021](0021-restricted-app-role-for-rls-enforcement.md)) - confirmed rotated in production too, not just CI/local dev.
- **Update**: `pictures` (one of the seven properties above) was later found unused in practice - `schemas/items.py`'s `_picture_refs` needed the owning `Payload` row's own id to build a content URL, not just its bytes, so it always re-walked `entity.information` independently rather than calling this property. Confirmed via grep before removing it, not assumed; six properties remain on `ItemViewMixin`.
