...
- **The subscriber sees and can drop it.** `GET /tenants/{tenant_id}/repositories` lists the repositories granted to a tenant, with each one's name, description, and `published_at`, for any member. It lists repositories the tenant has copied too, granted or not ([ADR 0119](0119-copying-a-repository-into-a-tenant.md)). `DELETE /tenants/{tenant_id}/repositories/{repository_id}` removes the grant, OWNER only. Removing a grant never touches what a copy already produced (RFC 0024 §6).
