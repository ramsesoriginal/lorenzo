...
- `GET /admin/users` - a real browse/search (`?q=` `ILIKE` against nickname/email), unlike ADR 0055's deliberately exact-match, no-search public lookup - this sits behind the new role instead of being open to any authenticated user, so a broader query surface is an acceptable, deliberate widening here.
