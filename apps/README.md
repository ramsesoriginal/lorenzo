...
- [`api`](api) — backend REST API. Infrastructure, plus the full domain model, a read-only REST API, and Authgear-backed auth (currently on `feat/auth-users`, not yet merged to `main`).
