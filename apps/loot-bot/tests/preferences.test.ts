...
describe("resolveCurrentCharacter", () => {
  ...
  it("returns the explicit value without touching the stored preference", async () => {
    const result = await resolveCurrentCharacter("user-1", "char-explicit");
  ...
  it("falls back to the stored preference when nothing explicit is given", async () => {
    ...
    const result = await resolveCurrentCharacter("user-1", undefined);
  ...
  it("falls back to the stored preference for a null explicit value too", async () => {
    ...
    const result = await resolveCurrentCharacter("user-1", null);
...
describe("resolveCurrentContainer", () => {
  ...
  it("returns the explicit value without touching the stored preference", async () => {
    const result = await resolveCurrentContainer("user-1", "container-explicit");
  ...
  it("falls back to the stored preference when nothing explicit is given", async () => {
    ...
    const result = await resolveCurrentContainer("user-1", undefined);
