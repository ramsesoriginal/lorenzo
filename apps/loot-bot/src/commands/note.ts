...
type Visibility = "public" | "private" | "gm-private";

/**
 * `/note` - adds a note to an item, with one of three visibilities. A GM
...
export const noteCommand: Command = {
  ...
  async execute(interaction, ctx) {
    ...
    try {
      ...
      if (visibility === "private" && authorCharacterId) {
        try {
          await client.addInformationKnower(tenantId, info.id, authorCharacterId, accessToken);
        } catch {
          await interaction.editReply(
            "Added the note, but couldn't grant your character visibility into it — it may currently be GM-only. Ask a GM to check.",
          );
          return;
        }
      }
