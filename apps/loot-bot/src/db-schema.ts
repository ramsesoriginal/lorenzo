 ...
 * means, passed as `GET /me/changes`'s own `since` (ADR 0097's addendum,
 * ADR 0099). One row per user, overwritten each time. This bot no longer
 * keeps the changes themselves - only this read marker, which has no API
