...
| `/drop` | GM-only: drops a pre-made loot container (autocompleted from unowned and your own containers, or paste an id or slug — slugs also show in `/inventory`, `/inspect`, and `/item`, [ADR 0087](../../docs/adr/0087-loot-bot-drop-autocomplete-and-slug-surfacing.md)) into the channel — take/claim (need or greed) /unclaim, then "apply claims" or "clear claims" in one batch |
