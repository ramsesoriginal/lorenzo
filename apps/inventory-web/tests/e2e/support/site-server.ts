...
// CLI directly under node, so stopping this process stops it too, and ignores Astro's
// preview lock: a lock left by an earlier run made it report "already running" and exit.
...
const args = [astro, 'preview', '--host', hostname, '--port', port, '--ignore-lock'];
const site = spawn(process.execPath, args, { env, stdio: 'inherit' });
