...
class GroupMember(Base):
    ...
    being.entity_id specifically, matching CharacterPlayer's own precedent
    that a "character_entity_id"-named column always targets Being, not a
    bare Entity.

    The CHECK below rejects the direct self-loop case where an entity that
    also happens to have a Being row (RFC 0001: an entity isn't limited to
    one concrete table) is listed as its own member - a real, reachable
    case, not hypothetical. No recursive-cycle trigger is needed unlike
    EntityPrototype: this table's bipartite shape (the group side ranges
    over all entities, the member side only over beings) can't otherwise
    form a multi-hop cycle.
    """
    ...
    character: Mapped[Being] = relationship(lazy="raise_on_sql", back_populates="group_links")
