...
Full read/write REST API, tenant-scoped under `/tenants/{tenant_id}/...` — see [ADR 0020](../../docs/adr/0020-rest-api-tenant-scoping-and-schemas.md) for the scoping/schema conventions. Routers under `src/lorenzo_api/routers/`: `entities`, `items`/`item_instances`, `payloads`/`information`, `stats`/`entity_stats`/`computed_stats`, `campaigns`, `campaign_invites`, `players`, `characters`, `groups`, `tenants`, `repositories` (repository publishing, grants, browsing, copying, and updates, [RFC 0024](../../docs/rfcs/0024-repositories.md)), `users`, `activity_log`, `knowledge`, plus `admin` (platform-operator only), `pictures`, and `invites` (the public side of campaign invite links). `/me` (current user + memberships), `/me/managed` (the tenants and campaigns the caller runs, [ADR 0086](../../docs/adr/0086-managed-scope-aggregate-and-notifications-since-filter.md)), `/me/changes` (what happened to things the caller's characters own or carry, [ADR 0099](../../docs/adr/0099-player-facing-change-feed.md)), `/invites/{token}` (an unauthenticated preview and an authenticated redeem, [ADR 0092](../../docs/adr/0092-campaign-invite-links.md)), and `/healthz`/`/readyz`/`/metrics` sit outside tenant scoping. How an owner exports a tenant through these reads is in [docs/operations/exporting-your-tenant.md](../../docs/operations/exporting-your-tenant.md). This list is deliberately not an exhaustive endpoint table — that duplicates and drifts from the generated source of truth; see [docs/reference/README.md](../../docs/reference/README.md) for the live `/docs` and the static `openapi.json` export, and [docs/adr/README.md](../../docs/adr/README.md) for the ADR-by-ADR history of how each surface landed.
